...
Definition x := fun z: Z => 1 + z = 3.
Definition eq_ind_2 := fun {A: Type} {x: A} (P: A -> Prop) (f: P x) {a: A} (e: x = a) =>
  match e with
  | eq_refl => f
  end.
Definition eq_sym_2 := fun {A: Type} {x y: A} (H: x = y) =>
match H in (_ = a) return (a = x) with
| eq_refl => eq_refl
end.
Check eq_sym_2.
Definition eq_ind_r_2 := fun {A: Type} {x: A} (P: A -> Prop) (f: P x) {a: A} (e: a = x) =>
  eq_ind_2 (fun y0: A => P y0) f (eq_sym_2 e).
Definition p := fun (st : state) (H: st "x" = 1) (H0: st "y" = 2) =>
  eq_ind_r_2 (fun z: Z => z + st "y" = 3) (eq_ind_r_2 (fun z: Z => 1 + z = 1 + 2) eq_refl H0) H : eval_expr_int [["x" + "y"]] st = 3.
Check p.
...
Ltac expr_auto := intros; unfold expr_int_equiv; intros; simpl; lia.
