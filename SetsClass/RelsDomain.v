...
Proof.
  ...
  + intros.
    ...
    - apply Sets_indexed_union_included; intros i0.
      ...
      * apply Sets_included_prop_inj; auto.
      * apply Rels_concat_mono_aux; [| reflexivity].
...
Ltac induction_1n H :=
  ...
  | @clos_refl_trans ?X ?_RELS ?_RELS_ID ?_SETS ?R0 =>
            change R' with (@Sets.omega_union _ _SETS (@nsteps X _RELS _RELS_ID R0)) in H at 1;
            revert_args_until_head P R' H;
            revert_EQ_until_head P R' H;
            revert H;
            match goal with
            | |- ?Pre -> ?Post => reduce_to_included (@Sets.omega_union _ _SETS (@nsteps X _RELS _RELS_ID R0)) Pre Post
            end;
